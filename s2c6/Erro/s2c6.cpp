...
int main()
{
  ...
  /*Metropolis Monte Carlo a N dimensões*/

  double integral_montecarlo_N;
  int dim;
  ...
  cout << " 1) sin(x) - 1 \n" <<  " 2) cos(x) - 2 \n" << " 3) exp(x) - 3 \n" << " 4) id(x) - 4 \n" << " 5) x² - 5 \n" << " 6) Função de onda quântica - 6 \n" << " 7) Monte Carlo a N dimensões para uma certa PDF - 7 \n\n"<< endl;
  cin >> f_user;
  ...
  if (f_user == 6)
   ...
   else if (f_user == 7)
    {
      ...
      cout << "Introduza o nº de dimensões (N):" << endl;
      cin >> dim;
      cout << endl;
	...
	{
	  matrix_MC[i] = new(nothrow) double[dim];
	  if(!(matrix_MC[i]))
	    {
	      cout << "Erro na alocação!" << endl;
	      return 1;
      ...
      markov_N(ro, a, n_confs, markovs, r, dim, matrix_MC);

      cout << " Estas são as configurações geradas: \n" << endl;

      for (int j = 0; j < dim; j++)
	{
	  cout << " Para a direção x" << j << ":\n\n" << endl;
	  for (int i = 0; i < n_confs; i++)
	    {
	      cout << " " << matrix_MC[i][j];
	      if ((i % 5) == 0)
      ...
      integral_montecarlo_N = integra_montecarlo_N(dim, matrix_MC, n_confs, func);

      cout << " O integral definido para N = " << dim << " dimensões da função a integrar é " << integral_montecarlo_N << "." << endl;
      /*
      for (int i = 0; i < n_confs; i++)
	{
